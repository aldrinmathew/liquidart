...
/// implementation like `ManagedAuthDelegate` from `package:liquidart/managed_auth.dart`.
///
...
class AuthServer implements AuthValidator, APIComponentDocumenter {
  ...
  ///
  /// It is preferable to use the implementation of [AuthServerDelegate] from 'package:liquidart/managed_auth.dart'. See
  ...
  /// Used during OpenAPI documentation.
  final APISecuritySchemeOAuth2Flow documentedPasswordFlow = APISecuritySchemeOAuth2Flow.empty()
    ..scopes = {};

  /// Used during OpenAPI documentation.
  final APISecuritySchemeOAuth2Flow documentedImplicitFlow = APISecuritySchemeOAuth2Flow.empty()
    ..scopes = {};
    ...
    final basic = APISecurityScheme.http("basic")
      ..description =
          "This endpoint requires an OAuth2 Client ID and Secret as the Basic Authentication username and password. "
              "If the client ID does not have a secret (public client), the password is the empty string (retain the separating colon, e.g. 'com.liquidart.app:').";
    context.securitySchemes.register("oauth2-client-authentication", basic);

    final oauth2 = APISecurityScheme.oauth2(
        {"authorizationCode": documentedAuthorizationCodeFlow, "password": documentedPasswordFlow})
  ...
  List<AuthScope> _validatedScopes(
    ...
    if (client.supportsScopes) {
      ...
      validScopes =
          requestedScopes.where((incomingScope) => client.allowsScope(incomingScope)).toList();
      ...
      if (!identical(validScopesForAuthenticatable, AuthScope.any)) {
        validScopes.retainWhere((clientAllowedScope) => validScopesForAuthenticatable
            .any((userScope) => clientAllowedScope.isSubsetOrEqualTo(userScope)));
  ...
  AuthToken _generateToken(int ownerID, String clientID, int expirationInSeconds,
  ...
  AuthCode _generateAuthCode(int ownerID, AuthClient client, int expirationInSeconds,
      {List<AuthScope> scopes}) {
    final now = DateTime.now().toUtc();
    return AuthCode()
