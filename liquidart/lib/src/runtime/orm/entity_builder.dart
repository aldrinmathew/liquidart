...
import 'package:liquidart/src/db/managed/attributes.dart';
...
import 'package:liquidart/src/db/managed/object.dart';
import 'package:liquidart/src/runtime/orm/property_builder.dart';
import 'package:liquidart/src/db/managed/relationship_type.dart';
import 'package:liquidart/src/runtime/orm_impl.dart';
...
class EntityBuilder {
  EntityBuilder(Type type)
      : instanceType = reflectClass(type),
        ...
        metadata = firstMetadataOfType<Table>(getTableDefinitionForType(type)) {
    name = _getName();
  ...
  Iterable<PropertyBuilder> _getTransientAttributes() {
    ...
    attributes.forEach((prop) {
      List<PropertyBuilder?> outList = out;
      final PropertyBuilder? complement =
          outList.firstWhere((pb) => pb!.name == prop.name, orElse: () => null);
      if (complement != null) {
