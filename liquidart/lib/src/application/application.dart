...
class Application<T extends ApplicationChannel> {
  ...
  ///
  /// This logger's name will appear as 'liquidart'.
  Logger logger = Logger("liquidart");
  ...
  Future start({int numberOfInstances = 1, bool consoleLogging = false}) async {
    if (server != null || supervisors.isNotEmpty) {
      throw StateError(
          "Application error. Cannot invoke 'start' on already running Liquidart application.");
    }
