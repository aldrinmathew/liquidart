...
class Request implements RequestOrResponse {
  ...
  ///
  /// You should typically not manipulate this property's [HttpRequest.response]. By default, Liquidart controls
  /// the response through its [Controller]s.
  ///
  /// If you wish to respond to a request manually - and prohibit Liquidart from responding to the request - you must
  ...
  ///
  /// Once this method has executed, the [Request] is no longer valid. All headers from [liquidartResponse] are
  /// added to the HTTP response. If [liquidartResponse] has a [Response.body], this request will attempt to encode the body data according to the
  /// Content-Type in the [liquidartResponse]'s [Response.headers].
  ///
  Future respond(Response liquidartResponse) {
    ...
    response.headers.add(
        HttpHeaders.contentTypeHeader, liquidartResponse.contentType.toString());
