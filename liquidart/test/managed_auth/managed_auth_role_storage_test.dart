...
void main() {
  ...
  setUpAll(() async {
    ...
    var clients = [
      AuthClient.withRedirectURI(
          "redirect", AuthUtility.generatePasswordHash("a", salt), salt, "http://a.com",
  ...
  group("Resource owner", () {
    test("AuthScope.Any allows all client scopes", () async {
      var t = await auth.authenticate(createdUsers.firstWhere((u) => u.role == "admin").username,
          User.defaultPassword, "redirect", "a",
    ...
    test("Restricted role scopes prevents allowed client scopes", () async {
      var t = await auth.authenticate(createdUsers.firstWhere((u) => u.role == "user").username,
          User.defaultPassword, "redirect", "a",
          requestedScopes: [AuthScope("user"), AuthScope("location:add")]);
    ...
    test(
        ...
        () async {
      try {
        await auth.authenticate(createdUsers.firstWhere((u) => u.role == "user").username,
            User.defaultPassword, "redirect", "a",
    ...
    test("Role that allows no scope prevents access token from being granted", () async {
      try {
        await auth.authenticate(createdUsers.firstWhere((u) => u.role == null).username,
            User.defaultPassword, "redirect", "a",
            requestedScopes: [AuthScope("location:add")]);
        expect(true, false);
      } on AuthServerException catch (e) {
    ...
    test("Client allows full scope, role restricts it to a subset, can only grant subset",
        () async {
      var t = await auth.authenticate(createdUsers.firstWhere((u) => u.role == "viewer").username,
          User.defaultPassword, "redirect", "a",
          requestedScopes: [AuthScope("user.readonly"), AuthScope("location:add:xyz")]);
    ...
    test("User allowed scopes can't grant higher privileges than client", () async {
      try {
        await auth.authenticate(createdUsers.firstWhere((u) => u.role == "invalid").username,
            User.defaultPassword, "redirect", "a",
  ...
  group("Refresh", () {
    test("Can't upgrade scope if it allowed by client, but restricted by role", () async {
      ...
      try {
        await auth.refresh(t.refreshToken, "redirect", "a", requestedScopes: [AuthScope("user")]);
        expect(true, false);
      } on AuthServerException catch (e) {
        expect(e.reason, AuthRequestError.invalidScope);
      }
  ...
  group("Auth code", () {
    ...
    test(
        ...
        () async {
      try {
        await auth.authenticateForCode(createdUsers.firstWhere((u) => u.role == "user").username,
            User.defaultPassword, "redirect",
            requestedScopes: [AuthScope("location:add")]);
        expect(true, false);
      } on AuthServerException catch (e) {
        expect(e.reason, AuthRequestError.invalidScope);
    ...
    test("Role that allows no scope prevents access token from being granted", () async {
      try {
        await auth.authenticateForCode(createdUsers.firstWhere((u) => u.role == null).username,
            User.defaultPassword, "redirect",
            requestedScopes: [AuthScope("location:add")]);
        expect(true, false);
      } on AuthServerException catch (e) {
    ...
    test("User allowed scopes can't grant higher privileges than client", () async {
      try {
        await auth.authenticateForCode(createdUsers.firstWhere((u) => u.role == "invalid").username,
            User.defaultPassword, "redirect",
            requestedScopes: [AuthScope("location")]);
        expect(true, false);
      } on AuthServerException catch (e) {
