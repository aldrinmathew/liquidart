...
void main() {
  group("Operations and security schemes", () {
    ...
    setUpAll(() async {
      doc = await Application.document(
          TestChannel, ApplicationOptions(), {"name": "Test", "version": "1.0"});
    });
    ...
    test("Authorizer adds Forbidden and Unauthorized to response components", () {
      ...
      expect(doc.components.responses["InsufficientScope"].content["application/json"].schema.type,
          APIType.object);
      expect(
          doc.components.responses["InsufficientScope"].content["application/json"].schema
  ...
  group("Controller Registration and Scopes", () {
    test("If no controllers added to channel, do not support have flows for oauth2 security type",
        () async {
    ...
    test("If only AuthController added to channel, do not support auth code flow", () async {
      final doc = await Application.document(
          AuthControllerOnlyChannel, ApplicationOptions(), {"name": "Test", "version": "1.0"});
      expect(doc.components.securitySchemes["oauth2"].flows["password"].refreshURL,
          Uri(path: "auth/token"));
      expect(doc.components.securitySchemes["oauth2"].flows["password"].tokenURL,
          Uri(path: "auth/token"));
    ...
    test(
        ...
        () async {
      final doc = await Application.document(
          ScopedControllerChannel, ApplicationOptions(), {"name": "Test", "version": "1.0"});
      expect(doc.components.securitySchemes["oauth2"].flows["password"].refreshURL,
          Uri(path: "auth/token"));
      expect(doc.components.securitySchemes["oauth2"].flows["password"].tokenURL,
          Uri(path: "auth/token"));
      ...
      expect(doc.components.securitySchemes["oauth2"].flows["authorizationCode"].refreshURL,
          Uri(path: "auth/token"));
      expect(doc.components.securitySchemes["oauth2"].flows["authorizationCode"].tokenURL,
          Uri(path: "auth/token"));
      expect(doc.components.securitySchemes["oauth2"].flows["authorizationCode"].authorizationURL,
          Uri(path: "auth/code"));
    });
    ...
    test("Referenced scopes for supported flows are available in oauth2 flow map", () async {
      ...
      expect(doc.components.securitySchemes["oauth2"].flows["password"].scopes,
          {"scope1": "", "scope2": ""});
      expect(doc.components.securitySchemes["oauth2"].flows["authorizationCode"].scopes,
          {"scope1": "", "scope2": ""});
    });
  });
}
    ...
    if (path.containsPathParameters([])) {
      return {
        "get": APIOperation("get/0", {
          "200": APIResponse("get/0-200",
              content: {"application/json": APIMediaType(schema: APISchemaObject.string())})
        }),
