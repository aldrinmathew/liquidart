...
void main() {
  ...
  test("If method has scopes, add them to list of scopes if does not exist in Authorizer", () {
    ...
    expect(doc.paths["/level1-authorizer"].operations["get"].security.first.requirements["oauth2"],
        ["level1"]);

    expect(doc.paths["/level1-authorizer"].operations["post"].security.length, 1);
    expect(
        doc.paths["/level1-authorizer"].operations["post"].security.first.requirements.length, 1);
    ...
    expect(
        doc.paths["/level1-authorizer"].operations["delete"].security.first.requirements.length, 1);
    expect(
        doc.paths["/level1-authorizer"].operations["delete"].security.first.requirements["oauth2"],
        ["level1", "level2"]);
    ...
    expect(doc.paths["/level1-authorizer"].operations["put"].security.first.requirements["oauth2"],
        ["level1"]);
  });
  ...
  test("If authorizer has less scope than method scope, method scope is used", () {
    ...
    expect(
        doc.paths["/level1-subscope-authorizer"].operations["get"].security.first.requirements
            .length,
        1);
    expect(
        doc.paths["/level1-subscope-authorizer"].operations["get"].security.first
            .requirements["oauth2"],
        ["level1:subscope"]);

    expect(doc.paths["/level1-subscope-authorizer"].operations["post"].security.length, 1);
    expect(
        doc.paths["/level1-subscope-authorizer"].operations["post"].security.first.requirements
            .length,
        1);
    expect(
        doc.paths["/level1-subscope-authorizer"].operations["post"].security.first
            .requirements["oauth2"],
        ["level1:subscope", "level2"]);

    expect(doc.paths["/level1-subscope-authorizer"].operations["put"].security.length, 1);
    expect(
        doc.paths["/level1-subscope-authorizer"].operations["put"].security.first.requirements
            .length,
        1);
    expect(
        doc.paths["/level1-subscope-authorizer"].operations["put"].security.first
            .requirements["oauth2"],
        ["level1"]);

    expect(doc.paths["/level1-subscope-authorizer"].operations["delete"].security.length, 1);
    expect(
        doc.paths["/level1-subscope-authorizer"].operations["delete"].security.first.requirements
            .length,
        1);
    expect(
        doc.paths["/level1-subscope-authorizer"].operations["delete"].security.first
            .requirements["oauth2"],
        ["level1", "level2"]);
  });
