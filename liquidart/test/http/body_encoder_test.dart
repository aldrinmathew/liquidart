...
void main() {
  ...
  test("Using an encoder that doesn't exist with a List<int> is OK", () async {
    var response = Response.ok(<int>[1, 2, 3, 4])..contentType = ContentType("foo", "bar");
    server = await bindAndRespondWith(response);
  ...
  test("String body, text content-type defaults content to latin1 but does not include in header",
      () async {
  ...
  test("A decoder with a match-all subtype will be used when matching", () async {
    ...
    var serverResponse = Response.ok("hello")..contentType = ContentType("b", "bar");
  ...
  test("A decoder with a subtype always trumps a decoder that matches any subtype", () async {
    ...
    CodecRegistry.defaultInstance.add(ContentType("a", "specific"), const JsonCodec());
  ...
  test("Using an encoder that blows up during encoded returns 500 safely", () async {
    CodecRegistry.defaultInstance.add(ContentType("application", "crash"), CrashingCodec());
    var serverResponse = Response.ok("abcd")..contentType = ContentType("application", "crash");
    server = await bindAndRespondWith(serverResponse);
  ...
  test("Encoder that doesn't net out with List<int> safely fails", () async {
    CodecRegistry.defaultInstance
        .add(ContentType("application", "baddata"), BadDataCodec(), allowCompression: false);
    var serverResponse = Response.ok("abcd")..contentType = ContentType("application", "baddata");
    server = await bindAndRespondWith(serverResponse);
  ...
  test("Encode with x-www-form-urlencoded", () {
    final codec = CodecRegistry.defaultInstance
  ...
  group("Compression", () {
    test("Content-Type that can be gzipped and request has Accept-Encoding will be gzipped",
        () async {
    ...
    test("Content-Type that can be gzipped but request does not have Accept-Encoding not gzipped",
        () async {
      server = await bindAndRespondWith(Response.ok({"a": "b"}));
    ...
    test("Unregistered content-type of List<int> does not get gzipped", () async {
      ...
      server = await bindAndRespondWith(Response.ok([1, 2, 3, 4])..contentType = ct);
      var req = await client.getUrl(Uri.parse("http://localhost:8888"));
      req.headers.clear();
    ...
    test("Content-type that can't be gzipped and Accept-Encoding accepts gzip, not gzipped",
        () async {
      var ct = ContentType("application", "3", charset: "utf-8");
      CodecRegistry.defaultInstance.add(ct, const JsonCodec(), allowCompression: false);
      server = await bindAndRespondWith(Response.ok({"a": "b"})..contentType = ct);
      var req = await client.getUrl(Uri.parse("http://localhost:8888"));
      req.headers.clear();
      req.headers.add("accept-encoding", "gzip");
      var resp = await req.close();
