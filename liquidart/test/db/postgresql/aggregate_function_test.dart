...
void main() {
  ...
  group("Average", () {
    ...
    test("Average produces average for int type", () async {
      ...
      expect(result,
          objects.fold(0, (p, n) => (p as dynamic) + n.i) / objects.length);
    });
    ...
    test("Average produces average for double type", () async {
      ...
      expect(
          result,
          objects.fold(0, (p, n) => (p as dynamic) + (n.d as dynamic)) /
              objects.length);
    });
    ...
    test("Average with predicate", () async {
      ...
      var result = await q.reduce.average((t) => t.i!);
      expect(result,
          objects.sublist(0, 5).fold(0, (p, n) => (p as dynamic) + n.i) / 5);
    });
  });
  ...
  group("Sum", () {
    ...
    test("Sum produces sum for double type", () async {
      ...
      expect(result, objects.fold(0, (p, n) => (p as double) + n.d!));
    });
