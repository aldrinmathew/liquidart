...
void main() {
  var store = PostgreSQLPersistentStore("dart", "dart", "localhost", 5432, "dart_test");
  ...
  test("Getting version number with 'blank' database (aka no version table yet) returns 0",
      () async {
    expect(await store.schemaVersion, 0);
  });

  test("Version table gets created on initiating upgrade if it doesn't exist", () async {
    await store.upgrade(Schema.empty(), [EmptyMigration()..version = 1], temporary: true);
  ...
  test("Trying to upgrade to version that is earlier than latest migration fails", () async {
    final s1 =
        await store.upgrade(Schema.empty(), [EmptyMigration()..version = 2], temporary: true);
    try {
      await store.upgrade(s1, [EmptyMigration()..version = 1], temporary: true);
      expect(true, false);
    } on MigrationException catch (e) {
  ...
  test("Apply more than one migration to new database", () async {
    await store.upgrade(
        Schema.empty(), [EmptyMigration()..version = 1, EmptyMigration()..version = 2],
        temporary: true);
    expect(await store.schemaVersion, 2);
  });
  ...
  test("Apply more than one migration to existing database", () async {
    ...
    expect(await store.schemaVersion, 2);
    await store.upgrade(
        Schema.empty(), [EmptyMigration()..version = 3, EmptyMigration()..version = 4],
        temporary: true);
    expect(await store.schemaVersion, 4);
  });

  test("Can apply an unencoded initial value", () async {
    await store.upgrade(Schema.empty(), [MCase11()..version = 1, MCase12()..version = 2],
        temporary: true);
    expect(await store.schemaVersion, 2);
