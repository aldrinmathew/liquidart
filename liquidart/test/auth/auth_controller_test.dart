...
void main() {
  ...
  setUp(() async {
    ...
    router = Router();
    router.route("/auth/token").link(() => AuthController(authenticationServer));
  ...
  group("Success Cases: password", () {
    ...
    test("Can authenticate with resource owner grant with client ID that has redirect url",
        () async {
  ...
  group("Success Cases: refresh_token", () {
    test("Confidental Client gets a new access token, retains same access token", () async {
      ...
      var resRefresh = await refresh(
          "com.stablekernel.app1", "kilimanjaro", refreshTokenMapFromTokenResponse(resToken));
      expect(
          resRefresh,
    ...
    test("If token is scoped and scope is omitted, get same token back", () async {
      ...
      var resRefresh = await refresh(
          "com.stablekernel.scoped", "kilimanjaro", refreshTokenMapFromTokenResponse(resToken));
      expect(
          resRefresh,
          hasResponse(200, body: {
            "access_token": isString,
  ...
  group("username Failure Cases", () {
    ...
    test("Username is empty returns 400", () async {
      var resToken =
          await grant("com.stablekernel.app1", "kilimanjaro", substituteUser(user1, username: ""));
      expect(resToken, hasResponse(400, body: {"error": "invalid_grant"}));
    });

    test("Username is missing returns 400", () async {
      var resToken =
          await grant("com.stablekernel.app1", "kilimanjaro", {"password": "doesntmatter"});
  ...
  group("password Failure Cases", () {
    ...
    test("password is missing returns 400", () async {
      var resToken =
          await grant("com.stablekernel.app1", "kilimanjaro", {"username": "${user1["username"]}"});
      expect(resToken, hasResponse(400, body: {"error": "invalid_request"}));
    });
  ...
  group("grant_type Failure Cases", () {
    ...
    setUp(() {
      client = Agent.onPort(8888)..setBasicAuthorization("com.stablekernel.app1", "kilimanjaro");
    });
  ...
  group("refresh_token Failure Cases", () {
    ...
    test("refresh_token appears more than once", () async {
      final Map<String, dynamic> grantMap =
          (await grant("com.stablekernel.app1", "kilimanjaro", user1)).body.as();
      final refreshToken = Uri.encodeQueryComponent(grantMap["refresh_token"] as String);

      var req = client.request("/auth/token")
        ..encodeBody = false
        ..body = utf8
            .encode("refresh_token=$refreshToken&refresh_token=abcdefg&grant_type=refresh_token")
        ..contentType = ContentType("application", "x-www-form-urlencoded");
      var resp = await req.post();
      expect(resp, hasResponse(400, body: {"error": "invalid_request"}));
      ...
      req = client.request("/auth/token")
        ..encodeBody = false
        ..body = utf8
            .encode("refresh_token=abcdefg&refresh_token=$refreshToken&grant_type=refresh_token")
        ..contentType = ContentType("application", "x-www-form-urlencoded");
      resp = await req.post();
      expect(resp, hasResponse(400, body: {"error": "invalid_request"}));
    });
  ...
  group("Authorization Header Failure Cases (refresh_token grant_type)", () {
    ...
    test("Confidential client has wrong secret", () async {
      var resp =
          await refresh("com.stablekernel.app1", "notright", {"refresh_token": refreshTokenString});
      expect(resp, hasResponse(400, body: {"error": "invalid_client"}));
    });
    ...
    test("Confidential Client ID doesn't exist", () async {
      var resp =
          await refresh("com.stablekernel.app123", "foo", {"refresh_token": refreshTokenString});
      expect(resp, hasResponse(400, body: {"error": "invalid_client"}));
    });

    test("client id is different than one that generated token", () async {
      var resp =
          await refresh("com.stablekernel.app2", "fuji", {"refresh_token": refreshTokenString});
...
Future<TestResponse> grant(String clientID, String clientSecret, Map<String, String> form) {
  Agent client = Agent.onPort(8888)..setBasicAuthorization(clientID, clientSecret);
