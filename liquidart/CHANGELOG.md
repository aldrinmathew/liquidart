...
- Removes `liquidart setup --heroku=<name>` and instead points to documentation.
...
- `package:liquidart/test` moved to `package:liquidart_test/liquidart_test`, which is a separate dependency from `liquidart`.
...
- Change default port for `liquidart serve` to 8888.
...
- Adds `liquidart db schema` to print an application's data model.
- Adds `liquidart document serve` that serves the API documentation for an application.
- Adds `--machine` flag to `liquidart` tool to only emit machine-readable output.
...
- Improves feedback and interface for `package:liquidart/test.dart`.
...
- The default template created by `liquidart create` is now mostly empty. Available templates can be listed with `liquidart create list-templates` and selected with the command-line option `--template`.
- Bug fixes where `liquidart auth` would fail to insert new Client IDs.
...
- Fixes issue with `liquidart serve` on Windows.

## 2.0.3

- Fixes issue with `liquidart document` for routes using `listen`
- Fixes issue when using `TestClient` to execute requests with public OAuth2 client
- Enables database migrations past the initial `liquidart db generate`.
...
- Change `liquidart serve` default port to 8081 so can develop in parallel to Angular2 apps that default to 8080
- Remove `SecurityContext` reference from `ApplicationConfiguration`. SSL configured via new `liquidart serve` arguments `ssl-key-path` and `ssl-certificate-path`, or overriding `securityContext` in `RequestSink`.
...
- Auth: Added `liquidart/managed_auth` library. Implements storage of OAuth 2.0 tokens using `ManagedObject`s. See API reference for more details.
...
- Tooling: Added `liquidart auth` tool to create client ID and secrets and add them to a database for applications using the `liquidart/managed_auth` package.
- Tooling: Added more user-friendly configuration options for `liquidart db` tool.
- Tooling: Added `liquidart setup --heroku` for setting up projects to be deployed to Heroku.
- Tooling: Added `liquidart serve` command for running Liquidart applications without having to write a start script.
- Tooling: Added `liquidart document` command to generate OpenAPI specification for Liquidart applications, instead of relying on a script that came with the template.
...
## 1.0.3
- Fix to allow Windows user to use `liquidart setup`.
...
- Add flags to `Process.runSync` that allow Windows user to use `liquidart` executable.
