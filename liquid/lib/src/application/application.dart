...
/// It is unlikely that you need to use this class directly - the `liquid serve` command creates an application object
/// on your behalf.
class Application<T extends ApplicationChannel> {
  ...
  ///
  /// This logger's name will appear as 'liquid'.
  Logger logger = Logger("liquid");
  ...
  Future start({int numberOfInstances = 1, bool consoleLogging = false}) async {
    if (server != null || supervisors.isNotEmpty) {
      throw StateError(
          "Application error. Cannot invoke 'start' on already running Liquid application.");
    }
  ...
  Future startOnCurrentIsolate() async {
    if (server != null || supervisors.isNotEmpty) {
      throw StateError(
          "Application error. Cannot invoke 'test' on already running Liquid application.");
    }
  ...
  ///
  /// This method is called by the `liquid document` CLI.
