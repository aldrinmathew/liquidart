...
class Request implements RequestOrResponse {
  ...
  ///
  /// You should typically not manipulate this property's [HttpRequest.response]. By default, Liquid controls
  /// the response through its [Controller]s.
  ///
  /// If you wish to respond to a request manually - and prohibit Liquid from responding to the request - you must
  ...
  ///
  /// Once this method has executed, the [Request] is no longer valid. All headers from [liquidResponse] are
  /// added to the HTTP response. If [liquidResponse] has a [Response.body], this request will attempt to encode the body data according to the
  /// Content-Type in the [liquidResponse]'s [Response.headers].
  ///
  Future respond(Response liquidResponse) {
    respondDate = DateTime.now().toUtc();

    final modifiers = _responseModifiers;
    _responseModifiers = null;
    modifiers?.forEach((modifier) {
      modifier(liquidResponse);
    });

    _Reference<String> compressionType = _Reference(null);
    var body = liquidResponse.body;
    if (body is! Stream) {
      ...
      body = _responseBodyBytes(liquidResponse, compressionType);
    }

    response.statusCode = liquidResponse.statusCode;
    liquidResponse.headers?.forEach((k, v) {
      response.headers.add(k, v);
    });

    if (liquidResponse.cachePolicy != null) {
      response.headers.add(HttpHeaders.cacheControlHeader,
          liquidResponse.cachePolicy.headerValue);
    }
    ...
    response.headers.add(
        HttpHeaders.contentTypeHeader, liquidResponse.contentType.toString());
    ...
    if (body is List<int>) {
      ...
      final bodyStream = _responseBodyStream(liquidResponse, compressionType);
      if (compressionType.value != null) {
        response.headers
            .add(HttpHeaders.contentEncodingHeader, compressionType.value);
      }
      response.headers.add(HttpHeaders.transferEncodingHeader, "chunked");
      response.bufferOutput = liquidResponse.bufferOutput;
