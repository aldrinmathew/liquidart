...
import 'package:liquid/src/cli/metadata.dart';
...
/// Used internally.
class CLITemplateCreator extends CLICommand with CLILiquidGlobal {
  CLITemplateCreator() {
  ...
  Future<int> handle() async {
    if (projectName == null) {
      printHelp(parentCommandName: "liquid");
      return 1;
    }
    ...
    displayProgress("See more templates with 'liquid create list-templates'");
    copyProjectFiles(destDirectory, templateSourceDirectory, projectName);

    createProjectSpecificFiles(destDirectory.path);
    if (liquidPackageRef.sourceType == "path") {
      final liquidLocation = liquidPackageRef.resolve().location;

      addDependencyOverridesToPackage(destDirectory.path, {
        "liquid": liquidLocation.uri,
        "liquid_test": liquidLocation.parent.uri.resolve("liquid_test/")
      });
    }
  ...
  String get detailedDescription {
    return "This command will use a template from the liquid package determined by either "
  ...
  String get description {
    return "Creates Liquid applications from templates.";
  }
}

class CLITemplateList extends CLICommand with CLILiquidGlobal {
  @override
  Future<int> handle() async {
  ...
  @override
  String get description {
    return "List Liquid application templates.";
  }
...
class CLILiquidGlobal {
  PubCache pub = PubCache();

  PackageRef get liquidPackageRef {
    return pub
        .getGlobalApplications()
        .firstWhere((app) => app.name == "liquid")
        .getDefiningPackageRef();
  }

  Uri get templateDirectory {
    return liquidPackageRef.resolve().location.uri.resolve("templates/");
  }
