...
class CLISetup extends CLICommand with CLIProject {
  ...
  @Option("heroku",
      help:
          "DEPRECATED. Please see https://liquid.io/docs/deploy/deploy_heroku/.")
  ...
  Future<int> handle() async {
    if (shouldSetupHeroku) {
      ...
      displayProgress("Please see https://liquid.io/docs/deploy/deploy_heroku/ for instructions.");
      return 0;
  ...
  Future<int> setupTestEnvironment() async {
    if (!hasPSQLCLI) {
      displayError(
          ...
          "you must do so to run tests in an Liquid application. For macOS users, "
    ...
    displayInfo(
        "Congratulations! Liquid applications can now be tested locally.");
