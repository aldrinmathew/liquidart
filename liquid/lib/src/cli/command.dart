...
import 'package:liquid/src/cli/running_process.dart';
...
abstract class CLICommand {
  ...
  Future<int> process(args.ArgResults results,
    ...
    try {
      ...
      if (showVersion) {
        outputSink.writeln("Liquid CLI version: $toolVersion");
        return 0;
      }

      if (!isMachineOutput) {
        displayInfo("Liquid CLI Version: $toolVersion");
      }
  ...
  Future determineToolVersion() async {
    try {
      var toolLibraryFilePath = (await Isolate.resolvePackageUri(
              currentMirrorSystem().findLibrary(#liquid).uri))
          .toFilePath(windows: Platform.isWindows);
      var liquidDirectory = Directory(FileSystemEntity.parentOf(
          FileSystemEntity.parentOf(toolLibraryFilePath)));
      var toolPubspecFile =
          File.fromUri(liquidDirectory.absolute.uri.resolve("pubspec.yaml"));
