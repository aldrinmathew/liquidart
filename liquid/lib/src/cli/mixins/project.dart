...
import 'package:liquid/src/cli/scripts/get_channel_type.dart';
...
abstract class CLIProject implements CLICommand {
  ...
  Version get projectVersion {
    if (_projectVersion == null) {
      ...
      final projectVersion =
          lockFileContents["packages"]["liquid"]["version"] as String;
  ...
  void preProcess() {
    try {
      if (!isMachineOutput) {
        displayInfo("Liquid project version: $projectVersion");
      }

      if (projectVersion?.major != toolVersion.major) {
        throw CLIException(
            "CLI version is incompatible with project liquid version.",
            instructions: [
              "Install liquid@${projectVersion?.toString()} or upgrade your project to liquid${toolVersion.toString()}."
            ]);
      }
    } on CLIException {
      rethrow;
