...
- Adds 'liquid build' command that generates an executable binary of an Liquid app, with some known issues
...
- [#723](https://github.com/stablekernel/liquid/pull/723) Fixes issue that prevented the `AuthServer` from granting tokens with sub-scopes when the servers `AuthServerDelegate.getAllowedScopes()` didn't return `AuthScope.any`.
...
- Removes `liquid setup --heroku=<name>` and instead points to documentation.
...
- `package:liquid/test` moved to `package:liquid_test/liquid_test`, which is a separate dependency from `liquid`.
...
- Change default port for `liquid serve` to 8888.
...
- Adds `liquid db schema` to print an application's data model.
- Adds `liquid document serve` that serves the API documentation for an application.
- Adds `--machine` flag to `liquid` tool to only emit machine-readable output.
...
- Improves feedback and interface for `package:liquid/test.dart`.
...
- The default template created by `liquid create` is now mostly empty. Available templates can be listed with `liquid create list-templates` and selected with the command-line option `--template`.
- Bug fixes where `liquid auth` would fail to insert new Client IDs.
...
- Fixes issue with `liquid serve` on Windows.

## 2.0.3

- Fixes issue with `liquid document` for routes using `listen`
- Fixes issue when using `TestClient` to execute requests with public OAuth2 client
- Enables database migrations past the initial `liquid db generate`.
...
- Change `liquid serve` default port to 8081 so can develop in parallel to Angular2 apps that default to 8080
- Remove `SecurityContext` reference from `ApplicationConfiguration`. SSL configured via new `liquid serve` arguments `ssl-key-path` and `ssl-certificate-path`, or overriding `securityContext` in `RequestSink`.
...
- Auth: Added `liquid/managed_auth` library. Implements storage of OAuth 2.0 tokens using `ManagedObject`s. See API reference for more details.
...
- Tooling: Added `liquid auth` tool to create client ID and secrets and add them to a database for applications using the `liquid/managed_auth` package.
- Tooling: Added more user-friendly configuration options for `liquid db` tool.
- Tooling: Added `liquid setup --heroku` for setting up projects to be deployed to Heroku.
- Tooling: Added `liquid serve` command for running Liquid applications without having to write a start script.
- Tooling: Added `liquid document` command to generate OpenAPI specification for Liquid applications, instead of relying on a script that came with the template.
...
## 1.0.3
- Fix to allow Windows user to use `liquid setup`.
...
- Add flags to `Process.runSync` that allow Windows user to use `liquid` executable.
