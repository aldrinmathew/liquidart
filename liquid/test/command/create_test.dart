...
void main() {
  ...
  group("Templates", () {
    ...
    test("Template gets generated from local path, project points to it",
      ...
      var liquidLocationString = File.fromUri(cli
              .agent.workingDirectory.uri
              .resolve("test_project/")
              .resolve(".packages"))
          .readAsStringSync()
          .split("\n")
          .firstWhere((p) => p.startsWith("liquid:"))
          .split("liquid:")
          .last;

      var path = path_lib.normalize(path_lib.fromUri(liquidLocationString));
    ...
    final templates = Directory("templates")
        ...
        .toList();
    final liquidPubspec = loadYaml(File("pubspec.yaml").readAsStringSync());
    final liquidVersion = Version.parse("${liquidPubspec["version"]}");

    for (var template in templates) {
      test("Templates can use 'this' version of Liquid in their dependencies",
          () {
        ...
        final projectVersionConstraint = VersionConstraint.parse(
            contents["dependencies"]["liquid"] as String);
        expect(projectVersionConstraint.allows(liquidVersion), true);
      });
