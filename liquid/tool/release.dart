...
class Runner {
  ...
  Uri baseReferenceURL = Uri.parse("https://www.dartdocs.org/documentation/liquid/latest/");
  ...
  Future<Directory> directoryWithBranch(String branchName) async {
    ...
    var process = await Process.start(
        "git",
        ["clone", "-b", branchName, "git@github.com:stablekernel/liquid.git", dir.path]);
    // ignore: unawaited_futures
    stderr.addStream(process.stderr);
    // ignore: unawaited_futures
    // stdout.addStream(process.stdout);
  ...
  Future<String> latestVersion() async {
    ...
    var response = await http.get(
        "https://api.github.com/repos/stablekernel/liquid/releases/latest",
  ...
  Future postGithubRelease(String version, String name, String description) async {
    ...
    if (!isDryRun) {
      var response = await http.post("https://api.github.com/repos/stablekernel/liquid/releases",
        headers: {
